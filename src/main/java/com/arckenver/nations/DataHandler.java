...
public class DataHandler
{
	...
	private static Hashtable<UUID, Hashtable<Vector3i, ArrayList<Nation>>> worldChunks;
	private static HashMap<UUID, Nation> lastNationWalkedOn;
	private static HashMap<UUID, Zone> lastZoneWalkedOn;
    private static Hashtable<UUID, PointCube> firstPointsCube;
    private static Hashtable<UUID, PointCube> secondPointsCube;
	...
	public static void load()
	{
		...
		firstPointsCube = new Hashtable<UUID, PointCube>();
		secondPointsCube = new Hashtable<UUID, PointCube>();
	...
	public static Nation getNation(Location<World> loc)
	{
		...
		Vector3i area = new Vector3i(IntMath.divide(loc.getBlockX(), 16, RoundingMode.FLOOR), IntMath.divide(loc.getBlockY(), 16, RoundingMode.FLOOR), IntMath.divide(loc.getBlockZ(), 16, RoundingMode.FLOOR));
		...
		{
			if (nation.getRegionCube().isInside(loc))
			{
				return nation;
			}
		}
	...
	public static boolean canClaim(Location<World> loc, boolean ignoreMinDistance, UUID toExclude)
	{
		for (Nation nation : nations.values())
		{
			if (!nation.getUUID().equals(toExclude) && nation.getRegionCube().distance(loc) < ConfigHandler.getNode("others", "minNationDistance").getInt())
			{
	...
	public static boolean canClaim(Cube cube, boolean ignoreMinDistance, UUID toExclude)
	{
		Optional<World> optWorld = Sponge.getServer().getWorld(cube.getWorld());
		if (!optWorld.isPresent())
		{
			return false;
		}
		World world = optWorld.get();
		return canClaim(world.getLocation(cube.getMaxX(), cube.getMaxY(), cube.getMaxZ()), ignoreMinDistance, toExclude) &&
				canClaim(world.getLocation(cube.getMaxX(), cube.getMaxY(), cube.getMinZ()), ignoreMinDistance, toExclude) &&
				canClaim(world.getLocation(cube.getMinX(), cube.getMaxY(), cube.getMaxZ()), ignoreMinDistance, toExclude) &&
				canClaim(world.getLocation(cube.getMinX(), cube.getMaxY(), cube.getMinZ()), ignoreMinDistance, toExclude) &&
				canClaim(world.getLocation(cube.getMaxX(), cube.getMinY(), cube.getMaxZ()), ignoreMinDistance, toExclude) &&
				canClaim(world.getLocation(cube.getMaxX(), cube.getMinY(), cube.getMinZ()), ignoreMinDistance, toExclude) &&
				canClaim(world.getLocation(cube.getMinX(), cube.getMinY(), cube.getMaxZ()), ignoreMinDistance, toExclude) &&
				canClaim(world.getLocation(cube.getMinX(), cube.getMinY(), cube.getMinZ()), ignoreMinDistance, toExclude);
	}

	public static void calculateWorldChunks()
	{
		worldChunks = new Hashtable<UUID, Hashtable<Vector3i, ArrayList<Nation>>>();
		for (Nation nation : nations.values())
		{
	...
	public static void addToWorldChunks(Nation nation)
	{
		...
		{
			System.out.println("check c.getWorld : " + c.getWorld());
			if (!worldChunks.containsKey(c.getWorld()))
			{
				worldChunks.put(c.getWorld(), new Hashtable<Vector3i, ArrayList<Nation>>());
			}
			Hashtable<Vector3i, ArrayList<Nation>> chunks = worldChunks.get(c.getWorld());
			for (int i = IntMath.divide(c.getMinX(), 16, RoundingMode.FLOOR); i < IntMath.divide(c.getMaxX(), 16, RoundingMode.FLOOR) + 1; i++)
			{
				for (int j = IntMath.divide(c.getMinY(), 16, RoundingMode.FLOOR); j < IntMath.divide(c.getMaxY(), 16, RoundingMode.FLOOR) + 1; j++)
				{
					for (int k = IntMath.divide(c.getMinZ(), 16, RoundingMode.FLOOR); k < IntMath.divide(c.getMaxZ(), 16, RoundingMode.FLOOR) + 1; k++)
					{
						Vector3i vect = new Vector3i(i, j, k);
    ...
    public static PointCube getFirstPointCube(UUID uuid)
    {
        if (ConfigHandler.getNode("others", "enableGoldenAxe").getBoolean(true))
        {
            return firstPointsCube.get(uuid);
        }
        ...
        return new PointCube(player.get().getWorld(), chunk.getX() * 16, chunk.getY() * 16, chunk.getZ() * 16);
    }

    public static void setFirstPointCube(UUID uuid, PointCube pointCube)
    {
        firstPointsCube.put(uuid, pointCube);
    }

    public static void removeFirstPointCube(UUID uuid)
    {
        firstPointsCube.remove(uuid);
    }

    public static PointCube getSecondPointCube(UUID uuid)
    {
        if (ConfigHandler.getNode("others", "enableGoldenAxe").getBoolean(true))
        {
            return secondPointsCube.get(uuid);
        }
        Optional<Player> player = Sponge.getServer().getPlayer(uuid);
        if (!player.isPresent())
        {
        ...
        Vector3i chunk = player.get().getLocation().getChunkPosition();
        return new PointCube(player.get().getWorld(), chunk.getX() * 16 + 15, chunk.getY() * 16 + 15, chunk.getZ() * 16 + 15);
    }

    public static void setSecondPointCube(UUID uuid, PointCube pointCube)
    {
        secondPointsCube.put(uuid, pointCube);
    }

    public static void removeSecondPointCube(UUID uuid)
    {
        secondPointsCube.remove(uuid);
    }
